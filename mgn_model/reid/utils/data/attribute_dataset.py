    ...
    for index, pid in enumerate(indices):
        ...
        for camid, cam_images in enumerate(pid_images):
            for fname in cam_images:
                ...
                data_list = name.split('_')
                x, y, color, car_type, roof, window = map(int, data_list[:6])
                logo = data_list[6]
                assert pid == x and camid == y
                if relabel:
                    ret.append((fname, index, camid, color, car_type, roof, window, logo))
                else:
                    ret.append((fname, pid, camid, color, car_type, roof, window, logo))
    return ret


class Attribute_Dataset(object):
