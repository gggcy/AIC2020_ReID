    ...
    for index, pid in enumerate(indices):
        ...
        for camid, cam_images in enumerate(pid_images):
            for fname in cam_images:
                ...
                x, y, color, car_type= map(int, data_list[:4])
                # logo = data_list[6]
                # print( pid,x , camid ,y)
                assert pid == x and camid == y
                if relabel:
                    ret.append((fname, index, camid, color, car_type))
                else:
                    ret.append((fname, pid, camid, color, car_type))
    return ret


class Attribute_Dataset_Simulation(object):
